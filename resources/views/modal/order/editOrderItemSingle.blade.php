 <div id='editOrderItem{{$item->id}}' class="modal fade">
    <div class="modal-dialog">
        <div class="modal-content">
        <!-- Заголовок модального окна -->
            <div class="modal-header">
            <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
            <h4 class="modal-title text-black">{{$item->name()}} - заказ № {{$order->id}}</h4>
            </div>
                ...
                <div class="container">
                    <div class='prices table-inline' style="width: 100%">
                                ...
                                <table>
                                    <tr>
                                        <td>Цена</td>
                                        <td><span id='currentPrice'>{{$item->item->getFirstPriceValue()}}</span> &#8381;</td>
                                    </tr>
                                    <tr>
                                        <td>Длина</td>
                                        <td id='currentDlina'>{{$item->item->getFirstLength()}} cм</td>
                                    </tr>
                                    <tr>
                                        <td>Количество</td>
                                        <td id='currentCount'>
                                ...
                                <form class='form' method="post" action="{{url('/basket/add')}}">
                                    ...
                                    <input type="hidden" id='form_dlina' name='dlina' value="{{$item->item->getFirstLength()}}">
